...
| `caxlsx` | 4.5 | сборка `routing_analytics.xlsx` в `bin/generate_excel_report.rb` (меню → **Form excel analytics**) |

`bundle install` дополнительно подтягивает транзитивные зависимости (`bigdecimal`, `drb`, `prism`, а для `caxlsx` — `nokogiri`, `rubyzip`, `htmlentities`, `marcel`) — руками их ставить не нужно.
...
- `data/` — исходные файлы для первичного импорта в БД (`providers.json`, `operations_history.csv`, `operations_queue_90.json`).
- `bin/` — исполняемые скрипты: `menu.rb` (интерактивное консольное меню, см. ниже), `import_data.rb` (импорт `data/*` + `config/business_parameters.yml` в БД), `route.rb` (обрабатывает очередь через Router, журналирует решения и состояние в БД, сразу собирает `routing_decisions_test.json`, затем пересчитывает аналитику провайдеров — см. `update_provider_minute_stats.rb` ниже), `build_decisions.rb` (пересобирает `routing_decisions_test.json` из уже заполненной БД без повторного роутинга), `build_report.rb` (собирает обязательный `routing_report_test.json` из БД), `generate_excel_report.rb` (собирает `routing_analytics.xlsx` из `routing_report_test.json`), `demo_rating.rb`/`demo_hard_filter.rb` (демонстрации без полного пайплайна), `analyze_db.rb`/`log_operations.rb` (аналитика по БД), `update_provider_minute_stats.rb` (пересчитывает и пишет в `providers` показатели вроде `actual_count_share_pct`/`approval_rate_pct` — класс `ProviderMinuteStats` в `lib/provider_minute_stats.rb`; вызывается автоматически после каждого успешного прогона `RoutingRun`, доступен и отдельно, вручную, с флагами `--window-seconds`/`--at`/`--dry-run`).
- `test/` — Minitest, зеркалирует структуру `lib/`.

Рейтинг, стратегии и hard-constraints читают только БД (`db/operations.db`) — файлы в `data/`/`config/business_parameters.yml` участвуют один раз, на этапе импорта. `routing_decisions_test.json`/`routing_report_test.json` — обязательные артефакты по итогам обработки `data/operations_queue_90.json` (или другой очереди, импортированной в БД); оба всегда собираются из БД (`DecisionsReader`/`CanonicalDatabaseAnalytics`), а не напрямую из решений `Router`'а в памяти.
...
1. **Start Route** — требует хотя бы одну активную стратегию и непустые `providers`/`operations_queue`/`operations_history`; иначе следующий экран объясняет, чего не хватает. При успехе сразу собирает оба обязательных артефакта — `routing_decisions_test.json` и `routing_report_test.json` (отдельного пункта меню для отчёта нет), а перед этим пересчитывает и записывает в `providers` показатели вроде `actual_count_share_pct`/`approval_rate_pct` (`ProviderMinuteStats`, за всё время по умолчанию).
2. **Data** → Update (добавляет/обновляет записи по естественному ключу, не дублирует) / Replace (очищает конкретную таблицу и грузит файл заново) / Clear (очищает всю БД).
3. **Switch strategies** — список всех 7 стратегий с отметкой `[x]`/`[ ]`; можно переключить несколько сразу одной строкой через пробел/запятую (например `1, 3, 5`).
...
6. **Form excel analytics** — запускает `bin/generate_excel_report.rb` поверх уже сформированного `routing_report_test.json` (без него — сообщение с просьбой сначала выполнить Start Route) и собирает `routing_analytics.xlsx` в корне проекта (6 листов: Dashboard, Providers, Routing, Segments, Period Comparison, Recommendations).
7. **Clear mode** — если включён, сразу после того как Start Route обработает очередь (и запишет оба файла), БД возвращается к исходным данным: все таблицы очищаются и заново заполняются из `data/*` + `config/business_parameters.yml`, как будто прогона не было. Правки `config/*.yml`, сделанные во время сессии, этим не откатываются — откатывается только содержимое БД.
