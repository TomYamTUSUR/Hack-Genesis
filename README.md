...
Механизм распределения выплат между провайдерами (СБП): hard-constraints + soft-goal стратегии рейтинга на Ruby + БД (SQLite/Sequel) для хранения провайдеров, очереди операций, истории и результатов роутинга, плюс интерактивная консоль (`bin/menu.rb`) для запуска и настройки без ручной правки файлов.
