...
#   1. пишет routing_decisions_test.json в корень репозитория (обязательный
#      артефакт, см. описание задания);
#   2. параллельно журналирует каждое решение в БД через уже готовый
#      RoutingAnalytics::DatabaseWriter#log_operations, чтобы bin/analyze_db.rb
#      мог построить отчёт по фактическому прогону очереди.
#
...
File.write(options[:output], JSON.pretty_generate(decisions.map(&:to_h)) + "\n", encoding: "UTF-8")
puts "#{decisions.size} решений записано в #{options[:output]}"
